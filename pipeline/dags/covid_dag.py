...
create_rds_conn = PythonOperator(
    ...
    op_kwargs={
        'conn_id': 'rds',
        'conn_type': env['RDS']['TYPE'],
        'host': env['RDS']['HOST'],
        'schema': env['RDS']['SCHEMA'],
        'login': env['RDS']['LOGIN'],
...
validate_fact_confirmed = PythonOperator(
    ...
    op_args=[
        ['staging.staging_global_confirmed', 'staging.staging_us_confirmed'],
        'fact.fact_metrics', 'confirmed'
    ]
)
...
validate_fact_deaths = PythonOperator(
    ...
    python_callable=validation_helper.validate_fact_metric,
    op_args=[
        ['staging.staging_global_deaths', 'staging.staging_us_deaths'],
        'fact.fact_metrics', 'deaths'
    ]
)
...
validate_fact_recovered = PythonOperator(
    ...
    python_callable=validation_helper.validate_fact_metric,
    op_args=[
        ['staging.staging_global_recovered'], 'fact.fact_metrics', 'recovered'
    ]
)
...
load_to_s3 >> create_rds_conn >> drop_staging >> create_tables >> stage_tables >> load_dim_tables >> load_temp_fact_tables >> load_fact_tables >> [
    validate_fact_confirmed, validate_fact_deaths, validate_fact_recovered
] >> load_bi_tables >> validate_bi_counts
